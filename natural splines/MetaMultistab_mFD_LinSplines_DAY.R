...
study <- read_excel("~/Desktop/phD/Meta_Multistab/Multistab_species_data_mfd.xlsx") %>%
  select(-c(35:51)) %>%
  rename(caseID = paper)

rawData <- read_excel("~/Desktop/phD/Meta_Multistab/Multistab_species_data_mfd.xlsx", 
                      sheet = "species data")
...
communityStab <- read_excel("~/Desktop/phD/Meta_Multistab/Multistab_species_data_mfd.xlsx", 
...
for(i in 1:length(USI)){
  ...
  if(dim(temp)[1]>2){#does the next step only if at least 3 data points are present
    AUC.RR<-myauc(temp$DAY, temp$RR,  from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                absolutearea = FALSE)
    AUC.pi<-myauc(temp$DAY, temp$delta.pi, from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                absolutearea = FALSE)
    AUC.totRR<-myauc(temp$DAY, temp$deltabm.tot, from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                   absolutearea = TRUE)
    AUC.RR.spline<-myauc(temp$DAY, temp$RR,  from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                       type = c("spline"),absolutearea = FALSE)
    AUC.pi.spline<-myauc(temp$DAY, temp$delta.pi, from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                       type = c("spline"),absolutearea = FALSE)
    AUC.totRR.spline<-myauc(temp$DAY, temp$deltabm.tot, from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                          type = c("spline"),absolutearea = TRUE)
    con.pi <- mean(temp$con.pi, na.rm = T)
    sum.con <- sum(temp$Con.M)
    stab.auc.day<-rbind(stab.auc.day,
...
#### remove duplicates ####
unique(stab.auc.day$caseID)

str(stab.auc)

data.plot.day <- stab.auc.day %>%
  distinct(caseID, studyID,system, organism, duration, dist.cat, open, species, resp.cat, sum.con,AUC.RR, AUC.RR.spline, AUC.pi.spline, AUC.pi, AUC.totRR, con.pi)%>%
  filter(resp.cat %in% c('abundance', 'biomass') ) %>%
  ungroup() %>%
  drop_na(AUC.pi)
names(data.plot.day)
hist(data.plot.day$AUC.RR)
hist(data.plot.day$AUC.RR.spline)


#### spline and linear methods ###
linearReg <- lm(data.plot.day$AUC.RR~data.plot$AUC.RR)
summary(linearReg)

linearRegPi <- lm(data.plot.day$AUC.pi~data.plot$AUC.pi)
summary(linearRegPi)

plot(data.plot.day$AUC.pi~data.plot$AUC.pi)
plot(data.plot.day$AUC.RR~data.plot$AUC.RR)

ggplot(data.plot.day, aes(y = AUC.pi, x = AUC.pi.spline))+
  geom_point(size = 1.5)+
  ...
  theme_bw()
#ggsave(plot = last_plot(), file = 'CorrelationAUC.pi_LinearNatural_day.png')

ggplot(data.plot.day, aes(y = AUC.RR, x = AUC.RR.spline))+
  geom_point(size = 1.5)+
  ...
  geom_abline(intercept = 0, slope = 1) +
  theme_bw()
#ggsave(plot = last_plot(), file = 'CorrelationAUC.RR_LinearNatural_day.png')



#1
raw <-ggplot(data.plot.day, aes(x=AUC.pi,y=AUC.RR, col=system))+
...
raw
#ggsave(plot = last_plot(), file = 'overview_Lindays.png', width = 8, height = 6)

#### COUnt SECTORS ####
data.plot.day$Sector<- NA
data.plot.day$Sector[data.plot.day$AUC.pi>0&data.plot.day$AUC.RR>0]<-1
data.plot.day$Sector[data.plot.day$AUC.pi>0&data.plot.day$AUC.RR<0]<-2
data.plot.day$Sector[data.plot.day$AUC.pi<0&data.plot.day$AUC.RR<0]<-3
data.plot.day$Sector[data.plot.day$AUC.pi<0&data.plot.day$AUC.RR>0]<-4
data.plot.day$Sector[data.plot.day$AUC.pi== 0 & data.plot.day$AUC.RR==0] <-0


sector.count<- data.plot.day%>%
  group_by( Sector) %>%
  count()%>%
  rename(N = n) %>%
  drop_na(Sector) %>%
...
ggplot(data.plot.day,aes(AUC.pi, AUC.RR, shape = system,color=system )) +
...
stab.auc.c.day <- data.frame()
for(i in 1:length(USIc)){
  temp<-communityStab1[communityStab1$caseID==USIc[i], ]#creates a temporary data frame for each case
  if(dim(temp)[1]>2){#does the next step only if at least 3 data points are present
    AUC.tot.RR<-auc(temp$DAY, temp$totRR, from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                    type = c("linear"),absolutearea = TRUE)
    AUC.totRR.spline <- auc(temp$DAY, temp$totRR, from = min(temp$DAY, na.rm = TRUE), to = max(temp$DAY, na.rm = TRUE),
                            type = c("spline"),absolutearea = TRUE)
    stab.auc.c.day<-rbind(stab.auc.c.day,
...
summary(stab.auc.c.day)

communityAUC.day <- distinct(stab.auc.c.day, caseID, resp.cat, AUC.tot.RR, AUC.totRR.spline)
ggplot(communityAUC.day, aes(x = AUC.tot.RR, y = AUC.totRR.spline))+
  geom_point(size = 1.5)+
  ...
  geom_abline(intercept = 0, slope = 1) +
  theme_bw()
#ggsave(plot = last_plot(), file = 'CorrelationAUC_OEVLinearNatural_DAY.png')

lm1 <-lm(communityAUC.day$AUC.totRR.spline~communityAUC.day$AUC.tot.RR)
summary(lm1)
plot( communityAUC$AUC.tot.RR~communityAUC.day$AUC.tot.RR )

## join Meta-Analysis stability values with sum of individual species if communitystab is not present ##
all.stab.auc <- left_join(data.plot.day,communityAUC.day,  by = c('caseID','resp.cat')) %>%
  mutate(OEV = ifelse(is.na(AUC.tot.RR), AUC.totRR, AUC.tot.RR )) %>%
  distinct(caseID,studyID, organism, system,duration, dist.cat, open, resp.cat, OEV)
...
alpha_fd_indices <- alpha.fd.multidim( # if you want to compute fric as richness/global richness, add mFD:: before alpha funciton
  sp_faxes_coord  = fspace$"sp_faxes_coord",
  asb_sp_w         = asb_sp_w,
...
Fdiv %>%
  ...
  facet_wrap(~resp.cat, scales = 'free_x') +
  theme_bw()+
  theme(legend.position = 'bottom')
...
ggsave(last_plot(), file = here('~/Desktop/phD/Meta_Multistab/FDiv_LinSplines_day.png'), width = 14, height = 10)
