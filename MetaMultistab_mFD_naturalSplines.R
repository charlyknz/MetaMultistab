...
for(i in 1:length(USI)){
  ...
  if(dim(temp)[1]>2){#does the next step only if at least 3 data points are present
    ...
    AUC.pi<-auc(temp$RD, temp$delta.pi, from = min(temp$RD, na.rm = TRUE), to = max(temp$RD, na.rm = TRUE),
                type = c("spline"),absolutearea = FALSE)
    AUC.totRR<-auc(temp$RD, temp$deltabm.tot, from = min(temp$RD, na.rm = TRUE), to = max(temp$RD, na.rm = TRUE),
                   type = c("spline"),absolutearea = TRUE)
    AUC.RR.spline<-auc(temp$RD, temp$RR,  from = min(temp$RD, na.rm = TRUE), to = max(temp$RD, na.rm = TRUE),
                type = c("linear"),absolutearea = FALSE)
    AUC.pi.spline<-auc(temp$RD, temp$delta.pi, from = min(temp$RD, na.rm = TRUE), to = max(temp$RD, na.rm = TRUE),
                type = c("linear"),absolutearea = FALSE)
    AUC.totRR.spline<-auc(temp$RD, temp$deltabm.tot, from = min(temp$RD, na.rm = TRUE), to = max(temp$RD, na.rm = TRUE),
...
linearReg <- lm(data.plot$AUC.RR~data.plot$AUC.RR.spline)
summary(linearReg)

linearRegPi <- lm(data.plot$AUC.pi~data.plot$AUC.pi.spline)
summary(linearRegPi)

AUC.RRcorr <- ggscatter(data.plot,cor.method = 'spearman',x = 'AUC.RR', y = 'AUC.RR.spline',
                        ylab = 'natural spline AUC.RR', xlab = 'Linear AUC.RR',add = 'reg.line', conf.int = T)  +
  stat_cor( label.x = -0.1) 

AUC.picorr <- ggscatter(data.plot,cor.method = 'spearman',x = 'AUC.pi', y = 'AUC.pi.spline',
                        ylab = 'natural spline AUC.pi', xlab = 'Linear AUC.pi',add = 'reg.line', conf.int = T)  +
  stat_cor( label.x = -0.1) 

#one plot for all correlation plot
Spline_corr <- cowplot::plot_grid(AUC.picorr,AUC.RRcorr,ncol = 1)
Spline_corr
...
raw
#ggsave(plot = last_plot(), file = 'overview_standardized.png', width = 8, height = 6)

#### COUnt SECTORS ####
data.plot$Sector<- NA
data.plot$Sector[data.plot$AUC.pi>0&data.plot$AUC.RR>0]<-1
data.plot$Sector[data.plot$AUC.pi>0&data.plot$AUC.RR<0]<-2
data.plot$Sector[data.plot$AUC.pi<0&data.plot$AUC.RR<0]<-3
data.plot$Sector[data.plot$AUC.pi<0&data.plot$AUC.RR>0]<-4
data.plot$Sector[data.plot$AUC.pi== 0 & data.plot$AUC.RR==0] <-0
...
ggplot(data.plot,aes(AUC.pi, AUC.RR, shape = system,color=system )) +
  ...
  theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank())+
  theme(plot.margin=unit(c(0.1,0.9,0.1,0.1),"cm"))
ggsave(plot = last_plot(), file = here('~/Desktop/PhD/Meta_Multistab/AUCpiAUCRR_naturalSplines.png'), width = 8, height = 5)
...
communityAUC <- distinct(stab.auc.c, caseID, resp.cat, AUC.tot.RR, AUC.totRR.spline)
...
all.stab.auc <- left_join(data.plot,communityAUC,  by = c('caseID','resp.cat')) %>%
  ...
  distinct(caseID,studyID, organism,species, system,duration, dist.cat, open, resp.cat, OEV)
...
Fdiv %>%
  select(caseID, fdis, fric, feve, sp_richn,studyID, organism, dist.cat, OEV, resp.cat,species,system,OrganismType) %>%
  gather(c(fdis, fric, feve), key = 'Indices', value = 'IndexValue') %>%
ggplot(., aes(x = IndexValue, y = OEV))+
  geom_hline(yintercept = 0)+
  geom_point(size = 2, alpha = .4)+
  labs(x = 'F Dispersion', y = 'OEV')+
  facet_grid(~Indices~resp.cat, scales = 'free') +
  theme_bw()+
  theme(legend.position = 'bottom')

fdis<-ggplot(Fdiv, aes(x = fdis, y = OEV, color = resp.cat))+
  geom_hline(yintercept = 0)+
  geom_point(size = 2, alpha = .4)+
  labs(x = 'F Dispersion', y = 'OEV')+
  theme_bw()+
...
fric<-ggplot(Fdiv, aes(x = fric, y = OEV, color = resp.cat))+
  geom_hline(yintercept = 0)+
  geom_point(size = 2, alpha = .4)+
...
feve<-ggplot(Fdiv, aes(x = feve, y = OEV, color = resp.cat))+
  geom_hline(yintercept = 0)+
  geom_point(size = 2, alpha = .4)+
...
sp_richn<-ggplot(Fdiv, aes(x = sp_richn, y = OEV, color = resp.cat))+
  geom_hline(yintercept = 0)+
  geom_point(size = 2, alpha = .4)+
...
ggsave(last_plot(), file = here('~/Desktop/phD/Meta_Multistab/FDiv_naturalsplines.png'), width = 14, height = 10)
...
#OEV
metadata1 <- metadata%>%
  gather(c(fdis,fric,feve,sp_richn), key = 'Fdiv', value = 'Fdiv_value')
test.complete.pi<-rma.mv(OEV,unweighted,
                      mods = ~Fdiv+resp.cat+dist.cat+system+open+organism+duration,
                      random = ~ 1 | caseID,
                      method="REML",data=metadata1)
summary(test.complete.pi)
complete.mod.pi<-tidy(summary(test.complete.pi))
complete.mod.pi$k<-test.complete.pi$k
complete.mod.pi$AIC<-test.complete.pi$fit.stats$REML[3]
